...
var _rsaPemToJwk = _interopRequireDefault(require("rsa-pem-to-jwk"));
...
var _sessionKeys = require("./session-keys");

/* eslint-env jest */
const instance = new _solidAuthClient.default();
/*
 * OIDC test data:
...
const pem = _fs.default.readFileSync(_path.default.join(__dirname, './id_rsa'));

const jwks = {
  keys: [(0, _rsaPemToJwk.default)( // the PEM-encoded key
  pem, // extra data for the JWK
  {
    kid: '1',
    alg: 'RS256',
    use: 'sig',
...
const sessionKey = JSON.stringify(_sessionKeys.sessionKeys.private);
...
beforeEach(() => {
  (0, _specHelpers.polyfillWindow)();

  _nock.default.disableNetConnect();
...
afterEach(() => {
  (0, _specHelpers.polyunfillWindow)();
...
const getStoredSession = () => (0, _session.getSession)((0, _storage.asyncStorage)(window.localStorage));
...
describe('login', () => {
  ...
  it('returns an anonymous auth response when no recognized auth scheme is present', async () => {
    expect.assertions(2);
    (0, _nock.default)('https://localhost').head('/').reply(200).get('/.well-known/openid-configuration').reply(404);
  ...
  describe('WebID-OIDC', () => {
    it('can log in with WebID-OIDC', async () => {
      expect.assertions(6);
      (0, _nock.default)('https://localhost/').get('/.well-known/openid-configuration').reply(200, oidcConfiguration).get('/jwks').reply(200, jwks).post('/register').reply(200, oidcRegistration);
...
describe('currentSession', () => {
  it('can find the current session if stored', async () => {
    expect.assertions(2);
    await (0, _session.saveSession)(window.localStorage)(fakeSession);
  ...
  describe('WebID-OIDC', () => {
    ...
    beforeEach(async () => {
      ...
      instance.on('session', (...params) => sessionEvents.push(params)); // To test currentSession with WebID-OIDC it's easist to set up the OIDC RP
      // client by logging in, generating the IDP's response, and redirecting
      // back to the app.

      (0, _nock.default)('https://localhost/').get('/.well-known/openid-configuration').reply(200, oidcConfiguration).get('/jwks').reply(200, jwks).post('/register').reply(200, oidcRegistration) // see https://github.com/anvilresearch/oidc-rp/issues/29
      .get('/jwks').reply(200, jwks);
      window.location.href = 'https://app.biz/page?foo=bar#the-hash-fragment';
      await instance.login('https://localhost'); // generate the auth response
      ...
      const alg = jwks.keys[0].alg;

      const idToken = _jsonwebtoken.default.sign({
      ...
      window.location.href = `${callbackUri}#` + `access_token=${accessToken}&` + `token_type=Bearer&` + `id_token=${idToken}&` + `state=${state}`;
    });
...
describe('trackSession', () => {
  ...
  it('yields an active session', async () => {
    expect.assertions(2);
    const session = {};
    await (0, _session.saveSession)(window.localStorage)(session);
    const callback = jest.fn();
    await instance.trackSession(callback);
    expect(callback).toHaveBeenCalledTimes(1);
...
describe('logout', () => {
  describe('WebID-OIDC', () => {
    ...
    beforeEach(async () => {
      ...
      (0, _nock.default)('https://localhost/').get('/.well-known/openid-configuration').reply(200, oidcConfiguration).get('/jwks').reply(200, jwks).post('/register').reply(200, oidcRegistration) // no luck, try with WebID-OIDC
      // see https://github.com/anvilresearch/oidc-rp/issues/29
      .get('/jwks').reply(200, jwks).get('/logout').reply(200);
      window.location.href = 'https://app.biz/page?foo=bar#the-hash-fragment';
      await instance.login('https://localhost'); // generate the auth response
...
describe.only('fetch', () => {
  const matchAuthzHeader = origin => headerVal => {
    const popToken = _jsonwebtoken.default.decode(headerVal[0].split(' ')[1]);

    return popToken.aud === origin && popToken.id_token === 'abc.def.ghi' && popToken.token_type === 'pop';
  };

  it('handles 401s from WebID-OIDC resources by resending with credentials', async () => {
    expect.assertions(1);
    await (0, _session.saveSession)(window.localStorage)(fakeSession);
    (0, _nock.default)('https://third-party.com').get('/protected-resource').reply(401, '', {
      'www-authenticate': 'Bearer scope="openid webid"'
    }).get('/protected-resource').matchHeader('authorization', matchAuthzHeader('https://third-party.com')).reply(200);
    const resp = await instance.fetch('https://third-party.com/protected-resource');
    expect(resp.status).toBe(200);
  });
  it('merges request headers with the authorization header', async () => {
    await (0, _session.saveSession)(window.localStorage)(fakeSession);
    (0, _nock.default)('https://third-party.com').get('/private-resource').reply(401, '', {
      'www-authenticate': 'Bearer scope="openid webid"'
    }).get('/private-resource').matchHeader('accept', 'text/plain').matchHeader('authorization', matchAuthzHeader('https://third-party.com')).reply(200);
    const resp = await instance.fetch('https://third-party.com/private-resource', {
      headers: {
        accept: 'text/plain'
      }
    });
  ...
  it('does not resend with credentials if the requested resource is public', async () => {
    expect.assertions(2);
    (0, _nock.default)('https://third-party.com').get('/public-resource').reply(200, 'public content', {
  ...
  describe('familiar domains with WebID-OIDC', () => {
    it('just sends one request when the RP is also the IDP', async () => {
      expect.assertions(1);
      await (0, _session.saveSession)(window.localStorage)(fakeSession);
      (0, _nock.default)('https://localhost').get('/resource').matchHeader('authorization', matchAuthzHeader('https://localhost')).reply(200);
    ...
    it('just sends one request to domains it has already encountered', async () => {
      expect.assertions(1);
      await (0, _session.saveSession)(window.localStorage)(fakeSession);
      await (0, _host.saveHost)(window.localStorage)({
        url: 'third-party.com',
        requiresAuth: true
      });
      (0, _nock.default)('https://third-party.com').get('/resource').matchHeader('authorization', matchAuthzHeader('https://third-party.com')).reply(200);
